...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "N_PROCESSES",
        help="Number of processes. If >1, call jax.distributed.initialize with that number of process. If -1 will call jax.distributed.initialize without any arsgument. So it should pick up the values from SLURM env variable.",
        default=1,
        type=int,
    )
    parser.add_argument("IP", help="IP of the master node for multi-node. Useless if using SLURM.", default="127.0.0.1", type=str, nargs="?")
    parser.add_argument("PROCESS_ID_INCREMENT", help="For multi-node only. Useless if using SLURM.", default=0, type=int, nargs="?")
