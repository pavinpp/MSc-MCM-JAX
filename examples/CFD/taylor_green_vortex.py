...
def taylor_green_initial_fields(xx, yy, u0, rho0, nu, time):
    ...
    rho = 1.0 - rho0 * u0**2 / 12.0 * (np.cos(2.0 * xx) + np.cos(2.0 * yy)) * np.exp(-4 * nu * time)
...
class TaylorGreenVortex(KBCSim):
    ...
    def initialize_macroscopic_fields(self):
        ux, uy, rho = taylor_green_initial_fields(xx, yy, vel_ref, 1, 0.0, 0.0)
